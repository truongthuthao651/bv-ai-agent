...
class FeedbackRequest(BaseModel):
    """``POST /feedback`` — a thumbs-down plus the user's written feedback.

    ``reason`` intentionally contains only the feedback the user chooses to
    submit; the application does not add question or answer text to the log.
    ``completion_id`` identifies the corresponding streamed answer.
    """

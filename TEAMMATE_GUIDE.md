...
| **Docker (dev)** | `docker compose up -d`; optional `docker-compose.gpu.yml` for NVIDIA |
...
.venv/bin/pytest tests/ -x -q      # tests only
...
- [x] Embedded Qdrant (native) + Docker server mode (dev)
...
- [ ] Reduce reranker latency on CPU-only machines (~50s/query measured on M1 Docker CPU)

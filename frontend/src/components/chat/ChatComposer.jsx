  ...
  return (
    <div style={{ background: "var(--surface-raised)", border: "1px solid " + (focus ? "var(--accent)" : "var(--border)"), borderRadius: "var(--radius-xl)", boxShadow: focus ? "var(--shadow-lg), 0 0 0 3px var(--accent-subtle)" : "var(--shadow-md)", padding: "var(--space-3)", transition: "border-color 150ms, box-shadow 150ms" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "var(--space-2)", marginTop: "var(--space-2)" }}>
        <button style={ctl} title="Attach PDF, Word, Excel or an image"><span style={{ fontSize: 14, lineHeight: 1 }}>+</span> Attach</button>
        <button style={ctl} title="Which document set is searched">Scope: {scope} <span style={{ color: "var(--text-muted)" }}>▾</span></button>
        <button style={ctl} title="Answering model — runs on the internal server">{model} <span style={{ color: "var(--text-muted)" }}>▾</span></button>
        <button onClick={onSend} disabled={!value} title="Send"
          style={{ marginLeft: "auto", width: 36, height: 36, borderRadius: "var(--radius-md)", border: "none", background: value ? "var(--accent)" : "var(--surface-sunken)", color: value ? "var(--text-on-accent)" : "var(--text-muted)", cursor: value ? "pointer" : "not-allowed", fontSize: 15, display: "flex", alignItems: "center", justifyContent: "center", transition: "background 150ms" }}>↑</button>
      </div>
    </div>
  );
}

...
**Evaluator environment:** macOS 24.6.0, Apple M1 Pro, 16 GB RAM, native stack running,
...
### 1.2 Ingestion pipeline end-to-end — **works for MD / XLSX / PDF / glossary; images not implemented**
...
### 1.3 Retrieval & answer quality — live queries
...
individually worth tens of seconds; the README's measured native Metal result shows the right
instinct was already applied once.
...
### 4.2 UX for non-technical staff — 7/10
...
| C2 test failures | Exact app-venv Python (`.venv/Scripts/python.exe` on Windows or `.venv/bin/python` on macOS/Linux) with `-m pytest tests/ -q` |
| C3 context window | `grep` for `llm_context_window`/`num_ctx` across `app/`; `ollama ps` CONTEXT column |
| Air-gap integrity | `grep` for all HTTP client calls in `app/`; `git ls-files` for tracked secrets; ignore-rule verification without inspecting protected data |

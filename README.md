...
No Docker required for deployment. Node/npm is only needed on the **dev machine** to rebuild the frontend (`frontend/` → `app/static/dist/`).
...
bash scripts/setup_native.sh  # one-time: venv, models, frontend build
...
.venv/bin/python scripts/seed_accounts.py you@baoviet.com PASS admin
docker compose up -d                        # optional Docker dev stack
```
